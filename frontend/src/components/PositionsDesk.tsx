...
export const PositionsDesk: React.FC<PositionsDeskProps> = ({
  ...
  const totalRealized = positions.reduce((acc, p) => acc + (p.realized_pnl || 0), 0);
      ...
      <div className="px-4 py-3 bg-obsidian-800/90 border-b border-obsidian-700/80 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-extrabold text-sm text-white">Positions Desk</span>
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-3 text-xs font-tabular">
            ...
            <div>
              ...
              <span className={`font-bold ${totalRealized >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
          ...
          <div className="divide-y divide-obsidian-700/40">
            {openPositions.map((pos) => {
              ...
              return (
                <div
                  key={pos.id || key}
                  className="px-4 py-3 hover:bg-obsidian-800/40 transition flex flex-col sm:flex-row sm:items-center justify-between gap-3"
                >
                  {/* Symbol & Product Type */}
                  <div className="flex items-center gap-3">
                    <div
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-slate-400 mt-0.5 font-tabular">
                        Qty: <strong className="text-slate-200">{Math.abs(pos.quantity)}</strong> &bull; Avg: ₹
                        {pos.average_price.toFixed(2)} &bull; LTP: ₹{pos.ltp?.toFixed(2) || pos.average_price.toFixed(2)}
                      </div>
                    </div>
                  </div>

                  {/* PnL & Exit Action */}
                  <div className="flex items-center justify-between sm:justify-end gap-4 font-tabular">
                    <div className="text-right">
                      <div className={`text-sm font-extrabold flex items-center justify-end gap-1 ${isProfit ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {isProfit ? <TrendingUp className="w-3.5 h-3.5" /> : <TrendingDown className="w-3.5 h-3.5" />}
                        <span>
                          {isProfit ? '+' : ''}₹{pos.unrealized_pnl?.toFixed(2) || '0.00'}
                        </span>
                      </div>
                      <div className={`text-[11px] font-semibold ${isProfit ? 'text-emerald-400/80' : 'text-rose-400/80'}`}>
                        {isProfit ? '+' : ''}{pos.pnl_percent?.toFixed(2) || '0.00'}%
                      </div>
                    </div>

                    <button
                      onClick={() => handleExit(pos.symbol, pos.product_type)}
                      disabled={isExiting}
                      className="px-3 py-1.5 rounded-lg bg-obsidian-700 hover:bg-rose-900/60 text-slate-200 hover:text-rose-200 text-xs font-semibold border border-obsidian-600 hover:border-rose-500/40 transition flex items-center gap-1.5"
                    >
            ...
            {closedPositions.length > 0 && (
              <div className="bg-obsidian-950/40 p-3">
                ...
                {closedPositions.map((pos) => (
                  ...
                  >
                    <span>{pos.symbol} ({pos.product_type})</span>
                    <span className={`font-bold ${pos.realized_pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      Realized: {pos.realized_pnl >= 0 ? '+' : ''}₹{pos.realized_pnl.toFixed(2)}
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-obsidian-800 border border-obsidian-700 rounded-2xl max-w-md w-full p-6 shadow-2xl">
            <div className="flex items-center gap-3 text-rose-400 mb-3">
              ...
              <h3 className="text-lg font-bold text-white">Square Off All MIS Positions?</h3>
            </div>
            <p className="text-slate-300 text-sm mb-6 leading-relaxed">
              This will place immediate market exit orders for all open Intraday (MIS) positions at current market prices and release margin.
            </p>
